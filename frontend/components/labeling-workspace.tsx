...
export default function LabelingWorkspace({
  ...
  const persistAnnotations = (updatedPolygons: any[] | null) => {
    try {
      const annotations = localStorage.getItem("hilips_annotations")
      const allAnnotations = annotations ? JSON.parse(annotations) : {}
      allAnnotations[selectedImage] = updatedPolygons ?? []
      localStorage.setItem("hilips_annotations", JSON.stringify(allAnnotations))
    } catch (error) {
  ...
  useEffect(() => {
    const storageKey = "hilips_classes"
    if (classes.length > 0) {
  ...
  const handleGeminiProcessing = async () => { // Renamed from handleGeminiSegmentation
    ...
    try {
      ...
      setPolygonData((prev) => {
        ...
        try {
          const annotationsRaw = localStorage.getItem("hilips_annotations")
